...
from PyQt5.QtWidgets import (
    ...
    QSizePolicy, QComboBox, QCheckBox, QSplitter, QSlider
)
...
class CalibComputeWorker(QThread):
    done = pyqtSignal(float, object, object)   # rms, K, dist
    fail = pyqtSignal(str)

    def __init__(self, objpoints: List[np.ndarray], imgpoints: List[np.ndarray], image_size: Tuple[int, int]):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            self.done.emit(float(rms), K, dist)
        except Exception as e:
            self.fail.emit(f"标定失败：{e}")


# =====================================================================
# 自动采集（从视频文件后台抽帧找角点）— 增强版
# =====================================================================
...
class AutoCollectWorker(QThread):
    ...
    def __init__(
        ...
        frame_max_width: int,
        dedup_px: float = 15.0,
        min_blur_var: float = 60.0,           # ✅ 模糊阈值（越大越严格）
        min_board_area_ratio: float = 0.02,   # ✅ 棋盘面积占比阈值
        cover_grid: Tuple[int, int] = (6, 4)  # ✅ 覆盖度网格
    ):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            while got < self.max_frames:
                ...
                self.log.emit(
                    f"[OK] {got}/{self.max_frames} frame#{idx} blur={blur_var:.1f} area={area_ratio:.3f} grid={cx},{cy}"
                )
...
class CameraPage(QWidget):
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        # 线程
        self.calib_thread: Optional[CalibComputeWorker] = None
        ...
        self.combo_view.setFixedWidth(220)
        ...
        # ✅ 擦除滑块
        row_wipe = QHBoxLayout()
        row_wipe.addWidget(self._lab("擦除位置："))
        self.slider_wipe = QSlider(Qt.Horizontal, self)
        self.slider_wipe.setRange(0, 100)
        self.slider_wipe.setValue(50)
        self.slider_wipe.setEnabled(False)
        row_wipe.addWidget(self.slider_wipe, stretch=1)
        self.lab_wipe = self._lab("50%")
        row_wipe.addWidget(self.lab_wipe)
        mid_layout.addLayout(row_wipe)
        self.slider_wipe.valueChanged.connect(lambda v: (self.lab_wipe.setText(f"{v}%"), self.refresh_display()))
        ...
        self.btn_auto = QPushButton("自动采集(视频)", self); self._style_button(self.btn_auto)
        ...
        self.btn_calib.clicked.connect(self.on_compute_calib_two_stage)  # ✅ 两阶段
    ...
    def _init_video_label(self, lab: QLabel, text: str):
        ...
        lab.setScaledContents(False)  # 不拉伸
    ...
    # ---------- 对比：工具 ----------
    def _hstack_same_h(self, left: np.ndarray, right: np.ndarray) -> np.ndarray:
        """左右拼接：先把两张图缩放到相同高度，再水平拼接"""
        ...
        if h <= 2:
            return left
        left2 = cv2.resize(left, (int(wl * (h / hl)), h), interpolation=cv2.INTER_AREA) if hl != h else left
        right2 = cv2.resize(right, (int(wr * (h / hr)), h), interpolation=cv2.INTER_AREA) if hr != h else right
        out = np.hstack([left2, right2])
        x = left2.shape[1]
        cv2.line(out, (x, 0), (x, out.shape[0] - 1), (255, 255, 255), 2)
        return out

    def _wipe_compare(self, a: np.ndarray, b: np.ndarray, pct: int) -> np.ndarray:
        """擦除对比：左边显示 b，右边显示 a（pct 决定分界线位置）"""
        ...
        if h <= 2 or w <= 2:
            return a
        a2 = cv2.resize(a, (w, h), interpolation=cv2.INTER_AREA) if (wa, ha) != (w, h) else a
        b2 = cv2.resize(b, (w, h), interpolation=cv2.INTER_AREA) if (wb, hb) != (w, h) else b
        x = int(np.clip(pct / 100.0 * w, 0, w))
        out = a2.copy()
        out[:, :x] = b2[:, :x]
        cv2.line(out, (x, 0), (x, h - 1), (255, 255, 255), 2)
        cv2.putText(out, "UNDIST", (12, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2, cv2.LINE_AA)
        cv2.putText(out, "RAW", (w - 80, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 200, 255), 2, cv2.LINE_AA)
        return out
    ...
    def refresh_display(self):
        ...
        # 启用/禁用擦除滑块
        self.slider_wipe.setEnabled(mode.startswith("对比（擦除"))
        ...
        # 对比（左右）
        if mode.startswith("对比（左右"):
            if ud is None:
                self.lab_video.setText("矫正失败（未加载参数或参数无效）")
                return
            raw = self.last_draw_bgr if self.last_draw_bgr is not None else self.last_raw_bgr
            ud_hud = self._overlay_calib_hud(ud.copy())
            cmp_img = self._hstack_same_h(raw, ud_hud)
            self._show_on_label(cmp_img, self.lab_video)
            return

        # 对比（擦除）
        if mode.startswith("对比（擦除"):
            if ud is None:
                self.lab_video.setText("矫正失败（未加载参数或参数无效）")
                return
            raw = self.last_draw_bgr if self.last_draw_bgr is not None else self.last_raw_bgr
            ud_hud = self._overlay_calib_hud(ud.copy())
            pct = int(self.slider_wipe.value())
            cmp_img = self._wipe_compare(raw, ud_hud, pct)
            self._show_on_label(cmp_img, self.lab_video)
            return
    ...
    def _show_on_label(self, bgr: np.ndarray, lab: QLabel):
        ...
        w = max(2, lab.width())
        h = max(2, lab.height())
        canvas = letterbox_to_size(bgr, w, h)   # 保持原始比例
    ...
    def on_compute_calib_two_stage(self):
        ...
        try:
            ...
            if len(errs) != len(self.calib_imgpoints):
                raise RuntimeError("重投影误差计算异常：帧数不一致")

            n = len(errs)
            order = np.argsort(errs)  # 小到大
            ...
            dropped_idx = order[keep:].tolist()  # noqa: F841
    ...
    def on_save_calib(self):
        if self.calib_K is None or self.calib_dist is None:
            ...
            return
        path, _ = QFileDialog.getSaveFileName(self, "保存相机参数", "camera_calib.yaml", "YAML Files (*.yaml *.yml)")
        if not path:
            return
...
class MainWindow(QMainWindow):
    def __init__(self, parent: Optional[QWidget] = None):
        super().__init__(parent)
        self.setWindowTitle("相机上位机（左上视频/左下日志/右控件更宽，标定增强+两阶段+对比）")
        self.resize(1700, 900)
        self.page = CameraPage(self)
        self.setCentralWidget(self.page)

    def closeEvent(self, event):
        try:
            if self.page:
                self.page.closeEvent(event)
        except Exception:
            pass
        super().closeEvent(event)
